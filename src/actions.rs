...
pub async fn get_hc_pod_ip(client: Client, target_node_name: &String, namespace: &str, hcport: i32) -> String {
    let mut t: i32 = 0;
        ...
        println!("  - {}", p.metadata.name.as_ref().unwrap());
           if let Some(spec) = p.spec {
            ...
            for container in spec.containers {
                if let Some(ports) = &container.ports {
                    for port in ports {
                        let port_test = port.container_port as i32;
                        if port_test == hcport {
                            if let Some(status) = &p.status {
                                if let Some(pod_ip) = &status.pod_ip {
                                    println!("Pod IP {}", pod_ip);
                                    hcpod_ip = pod_ip.to_string();
                                } else {
                                    println!("Pod IP not yet assigned.");
                                    hcpod_ip = "0.0.0.0".to_string();
                                }
                            }
                        }
                    }
